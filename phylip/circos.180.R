...
names(d) <- c("name", "x", "y")
scale.alpha <- 1/(max(d$x)+1) * 1*pi
offset.r <- 600
d$r <- offset.r + d$y
d$alpha <- d$x*scale.alpha-pi/2
l <- read.table('data.one.lines')
names(l) <- c("line", "x0", "y0", "x1", "y1")
l$r0 <- offset.r + l$y0
l$alpha0 <- l$x0*scale.alpha-pi/2
l$r1 <- offset.r + l$y1
l$alpha1 <- l$x1*scale.alpha-pi/2

radius <- max(d$r)*1+50

png('pars.one.circos-180.png', width=1000, height=500, type='cairo', antialias='subpixel')

par(mar=c(1,1,1,1))
plot(d$r*sin(d$alpha), d$r*cos(d$alpha), xlim=c(-radius, radius), ylim=c(0, radius), pch=20, cex=1, ann=F, axes=F)
rs <- max(d$r)
for (i in 1:nrow(d)) {
