...
png('pars.all.circos.huge.png', width=5500, height=5500, type='cairo', antialias='subpixel')
...
for (i in 1:nrow(d)) {
  ...
  rinc <- radius*0.0125
  r <- radius
  text(r*sin(d$alpha[i]), r*cos(d$alpha[i]), d$fs[i], adj=c(adjx, 0.4), srt=alpha, family='DejaVu Sans Condensed')
  r <- r + 4.5*rinc
  text(r*sin(d$alpha[i]), r*cos(d$alpha[i]), paste('2.6.',d$major[i], sep=''), adj=c(1-adjx, 0.4), srt=alpha, family='DejaVu Sans Condensed')
  ...
  segments((radius+rinc)*sin(d$alpha[i]),   (radius+rinc)*cos(d$alpha[i]),
           (radius+2*rinc)*sin(d$alpha[i]), (radius+2*rinc)*cos(d$alpha[i]), col=my.colors58[d$fs[i]], lwd=10)
  rspace <- rinc
  segments((radius+rspace+2*rinc)*sin(d$alpha[i]), (radius+rspace+2*rinc)*cos(d$alpha[i]),
           (radius+rspace+3*rinc)*sin(d$alpha[i]), (radius+rspace+3*rinc)*cos(d$alpha[i]), col=my.colors30[d$major[i]], lwd=10)
}
...
for (i in 1:nrow(l)) {
  if (l$r0[i] == l$r1[i]) {
    ...
    segments(l$r0[i]*sin(l$alpha0[i]), l$r0[i]*cos(l$alpha0[i]), l$r1[i]*sin(l$alpha1[i]), l$r1[i]*cos(l$alpha1[i]), lwd=3)
  }
}
