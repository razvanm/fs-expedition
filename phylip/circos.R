...
names(d) <- c("name", "x", "y")
scale.alpha <- 1/(max(d$x)+1) * 1.9*pi
offset.r <- 300
d$r <- offset.r + d$y
...
png('pars.one.circos.png', width=800, height=800, type='cairo', antialias='subpixel')

par(mar=c(1,1,1,1))
plot(d$r*sin(d$alpha), d$r*cos(d$alpha), xlim=c(-radius, radius), ylim=c(-radius, radius), pch=20, cex=1, ann=F, axes=F)
